...
testKaratsuba :: Spec
testKaratsuba = do
    it "Can shiftL a Natural by 0 bits." $
        let nat = mkNatural [ 1, 0, 4 * 2, 0, 16 * 3, 0, 64 * 4, 0, 256 * 5, 0, 256 * 4 * 6 ]
        in show (shiftLNatural nat 0) `shouldBe` show nat
    ...
    it "kShiftedAdd works arbitrary overlap A." $ do
        let n2 = mkNatural (fromString "0x10000000b")
            n1 = mkNatural (fromString "0x0")
            n0 = mkNatural (fromString "0x6c000000280000009000000380000004c000000000000002")
        show (kShiftedAdd 1 n2 n1 n0) `shouldBe` show (kShiftedAddSlow 1 n2 n1 n0)

    it "kShiftedAdd works arbitrary overlap B." $ do
        let n2 = mkNatural (fromString "0xc00000098000001a")
            n1 = mkNatural (fromString "0x5c000000d00000001")
